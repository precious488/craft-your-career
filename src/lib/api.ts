...
export async function apiFetch<T = unknown>(
  ...
  if (res.status === 401) {
    ...
    if (!refreshToken) {
      clearTokens()
      throw new Error('Session expired') // ← remove window.location.href
    }
    ...
    try {
      ...
      if (!refreshRes.ok) {
        clearTokens()
        throw new Error('Session expired') // ← remove window.location.href here too
      }
      // ... rest stays the same
...
export const resumeAPI = {
  ...
  get: (id: string) =>
    apiFetch<{ success: boolean; data: ResumeFromAPI }>(`/resumes/${id}`),
  create: (data: Partial<ResumeFromAPI>) =>
    apiFetch<{ success: boolean; data: ResumeFromAPI }>('/resumes', {
      method: 'POST',
  ...
  duplicate: (id: string) =>
    apiFetch<{ success: boolean; data: ResumeFromAPI }>(
...
// ─── AI API ───────────────────────────────────────────────────
export const aiAPI = {
  improveSummary: (payload: {
    currentSummary: string
    jobTitle?: string
    skills?: string[]
  }) =>
    apiFetch<{ success: boolean; data: { improved: string } }>(
      '/ai/improve-summary',
      {
        method: 'POST',
        body: JSON.stringify(payload),
  ...
  generateBullets: (payload: {
    ...
    apiFetch<{ success: boolean; data: { bullets: string[] } }>(
      '/ai/bullet-points',
      {
        method: 'POST',
        body: JSON.stringify(payload),
  ...
  suggestSkills: (payload: {
    ...
    apiFetch<{ success: boolean; data: { skills: string[] } }>(
      '/ai/suggest-skills',
      {
        method: 'POST',
        body: JSON.stringify(payload),
  ...
  autocomplete: (payload: {
    field: 'summary' | 'description' | 'projectDescription'
    partialText: string
    context?: object
  }) =>
    apiFetch<{ success: boolean; data: { suggestion: string } }>(
      '/ai/autocomplete',
      {
        method: 'POST',
        body: JSON.stringify(payload),
...
// ─── REPLACE the feedbackAPI object in src/lib/api.ts with this expanded version ───

// ─── ADD/REPLACE in src/lib/api.ts ───────────────────────────────
// Replace the existing FeedbackPayload and FeedbackEntry interfaces
// and the feedbackAPI object with this:
...
export const feedbackAPI = {
  submit: (payload: FeedbackPayload) =>
    apiFetch<{ success: boolean; data: FeedbackEntry }>('/feedback', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),

  myFeedback: () =>
    apiFetch<{ success: boolean; data: FeedbackEntry[] }>('/feedback/me'),

  published: () =>
    apiFetch<{ success: boolean; data: FeedbackEntry[] }>(
      '/feedback/published',
    ),
  ...
  adminSetPublished: (id: string, isPublished: boolean) =>
    apiFetch<{ success: boolean; data: FeedbackEntry }>(
      `/feedback/admin/${id}/publish`,
      {
        method: 'PATCH',
