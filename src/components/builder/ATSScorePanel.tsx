 ...
 * Usage in CVWorkspace.tsx: <ATSScorePanel resumeData={draft} cvId={draft.id} />
 */
...
export default function ATSScorePanel({ resumeData, cvId }: Props) {
  ...
  async function runAnalysis() {
    ...
    setError('')
    try {
      const { data } = await atsAPI.analyze(
        resumeData,
                ...
                >
                  ...
                  <div className='flex items-center gap-4'>
                    ...
                    <div className='flex-1 space-y-2'>
                      <BarScore
                        label='Keyword Match'
                        value={result.breakdown.keywordMatch}
                      />
                      <BarScore
                        label='Completeness'
                        value={result.breakdown.sectionCompleteness}
                      />
                      <BarScore
                        label='Action Verbs'
                      ...
                      <BarScore
                        label='Achievements'
                        value={result.breakdown.quantifiableAchievements}
                      />
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className='flex flex-wrap gap-1'>
                        {result.matchedKeywords.slice(0, 10).map((kw) => (
                          <span
                            key={kw}
                            className='text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-full'
                          >
                            {kw}
                          </span>
                        ))}
                    ...
                    <div>
                      ...
                      <div className='flex flex-wrap gap-1'>
                        {result.missingKeywords.map((kw) => (
                          <span
                            key={kw}
                            className='text-[10px] bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-0.5 rounded-full'
                          >
                            {kw}
                          </span>
                        ))}
