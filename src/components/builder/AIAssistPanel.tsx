...
export function SummaryAssist({
  ...
  //       <span>
  //         AI summary improvement is a{' '}
  //         <span className='text-primary font-medium'>Pro</span> feature
  //       </span>
  //     </div>
  //   )
  // }
...
export function BulletAssist({
  ...
  async function generate() {
    ...
    setIsOpen(true)
    try {
      const { data } = await aiAPI.generateBullets({
        position,
        company,
        existingDescription,
        numberOfPoints: 3,
      ...
      {isOpen && bullets.length > 0 && (
        <div className='mt-2 space-y-1.5'>
          {bullets.map((bullet, i) => (
            <div
              key={i}
              className='flex items-start gap-2 bg-primary/5 rounded-lg px-3 py-2'
            >
