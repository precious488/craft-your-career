    ...
    <nav className='fixed top-0 left-0 right-0 z-50  backdrop-blur-xl border-b border-border/50'>
      <div className='container mx-auto px-4 h-16 flex items-center justify-between'>
        <Link to='/' className='flex items-center gap-2 font-bold text-xl'>
          ...
          <span className='text-foreground font-serif'>ResumeAI</span>
        </Link>
          ...
          {isAuthenticated ? (
            <div className='relative'>
              <button
                ...
                className='flex items-center gap-2 text-sm text-foreground hover:text-primary transition-colors'
              >
                <div className='w-7 h-7 rounded-full bg-rust flex items-center justify-center text-paper text-xs font-semibold'>
                  {user?.fullName?.[0]?.toUpperCase() ?? 'U'}
                </div>
                <span className='font-medium'>
                  {user?.fullName?.split(' ')[0]}
              ...
              {isAuthenticated ? (
                ...
                <>
                  ...
                  <Link to='/register' onClick={() => setMobileOpen(false)}>
                    <Button
                      size='sm'
                      className='w-full bg-rust hover:bg-rust-dark text-paper'
                    >
                      Get Started Free
                    </Button>
                  </Link>
